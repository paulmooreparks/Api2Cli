        ...
        )
    {
        ...
        foreach (Match match in matches) {
            ...
            if (namespacePrefix == "[env]::" || string.IsNullOrEmpty(namespacePrefix)) {
                ...
                var api2CliSettingsDirectory = settingsService.Api2CliSettingsDirectory;
                string filePath = variable;

                try {
                    if (!Path.IsPathRooted(filePath)) {
                        if (!string.IsNullOrWhiteSpace(api2CliSettingsDirectory) && Directory.Exists(api2CliSettingsDirectory)) {
                            Directory.SetCurrentDirectory(api2CliSettingsDirectory);
                        }
