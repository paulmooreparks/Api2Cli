...
[TestClass]
public class PackagesDirArgTests
{
    ...
    [TestMethod]
    public void PackagesArg_FilePath_FailsWithClearMessage()
    {
        var tempDir = Directory.CreateTempSubdirectory();
        try
        {
            var fileLike = Path.Combine(tempDir.FullName, "pkg.txt");
            File.WriteAllText(fileLike, "# sentinel\n");

            var (code, stdout, stderr) = RunCli(
                "--packages", fileLike,
                "--config", tempDir.FullName,
            ...
            Assert.AreNotEqual(0, code, $"CLI should fail when --packages points to a file. Output:\nSTDOUT:\n{stdout}\nSTDERR:\n{stderr}");
            StringAssert.Contains(stderr, "--packages must be a directory", "Error should explain that --packages must be a directory");
        }
        finally
        {
